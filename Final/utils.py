...
def evaluate_portfolio_performance(agent, logger):
    ...
    logger.info("--------------------------------")
    logger.info('Portfolio Value:        Rs{:.2f}'.format(agent.portfolio_values[-1]))
    logger.info('Portfolio Balance:      Rs{:.2f}'.format(agent.balance))
    logger.info('Portfolio Stocks Number: {}'.format(len(agent.inventory)))
    logger.info('Total Return:           Rs{:.2f}'.format(portfolio_return))
...
def plot_portfolio_transaction_history(stock_name, agent):
	...
	plt.title('{} Total Return on {}: Rs{:.2f}'.format(agent.model_type, stock_name, portfolio_return))
...
def plot_portfolio_performance_comparison(stock_name, agent):
	...
	plt.plot(dates, agent.portfolio_values, color='green', label='{} Total Return: Rs{:.2f}'.format(agent.model_type, agent_return))
	plt.plot(dates, buy_and_hold_portfolio_values, color='blue', label='{} Buy and Hold Total Return: Rs{:.2f}'.format(stock_name, buy_and_hold_return))
	...
	if '^GSPC' not in stock_name:
		...
		plt.plot(dates, GSPC_buy_and_hold_portfolio_values, color='red', label='S&P 500 2018 Buy and Hold Total Return: Rs{:.2f}'.format(GSPC_buy_and_hold_return))
...
def plot_all(stock_name, agent):
    ...
    buy_prices = [df.iloc[t, 4] for t in agent.buy_dates]
    sell_prices = [df.iloc[t, 4] for t in agent.sell_dates]
    ax[0].set_title('{} Total Return on {}: Rs{:.2f}'.format(agent.model_type, stock_name, portfolio_return))
    ...
    ax[1].plot(dates, agent.portfolio_values, color='green', label='{} Total Return: Rs{:.2f}'.format(agent.model_type, agent_return))
    ax[1].plot(dates, buy_and_hold_portfolio_values, color='blue', label='{} Buy and Hold Total Return: Rs{:.2f}'.format(stock_name, buy_and_hold_return))
    # compare with S&P 500 performance in 2018 if stock is not S&P 500
    if '^GSPC' not in stock_name:
    	dates, GSPC_buy_and_hold_portfolio_values, GSPC_buy_and_hold_return = buy_and_hold_benchmark('^GSPC_2018', agent)
    	ax[1].plot(dates, GSPC_buy_and_hold_portfolio_values, color='red', label='S&P 500 2018 Buy and Hold Total Return: Rs{:.2f}'.format(GSPC_buy_and_hold_return))
    ax[1].set_ylabel('Portfolio Value (Rs)')
