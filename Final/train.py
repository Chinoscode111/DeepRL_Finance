...
def buy(t):
    if agent.balance > stock_prices[t]:
        ...
        return 'Buy: RS{:.2f}'.format(stock_prices[t])
...
def sell(t):
    if len(agent.inventory) > 0:
        ...
        global reward
        reward = profit
        return 'Sell: Rs{:.2f} | Profit: Rs{:.2f}'.format(stock_prices[t], profit)
...
logging.info('Initial Portfolio Value: Rs{:,}'.format(initial_balance))
...
for e in range(1, num_episode + 1):
    ...
    if e % 5 == 0:
        if model_name == 'DQN':
            agent.model.save('saved_models/DQN_ep_self' + str(e) + '.h5')
